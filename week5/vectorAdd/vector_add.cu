...
const int MAX_SIZE=10000000; //만 -> 천만, gpu가 cpu에 비해 성능이 10배 좋음
...
float inputA[MAX_SIZE]; //cpu 데이터 포인터
float inputB[MAX_SIZE]; //cpu 데이터 포인터
...
int main(){
	...
	err = cudaMemcpy(gpuAns, d_c, arraySize, cudaMemcpyDeviceToHost); //gpu에서 계산한 데이터를 cpu로 옮기는 작업
	checkCudaError(err);
